...
export class DashBoardPage {
  ...
  quantity_product_cart_first: number;
  readonly page: Page;
  ...
  async init_cart() {
    let quantity_product_cart: any = await this.cart.textContent();
  ...
  async validationAddProduct() {
    ...
    const second_quantity: any = await this.cart.textContent();
    console.log(second_quantity);
    //expect(second_quantity).toBeGreaterThan(this.quantity_product_cart_first);
    if (parseInt(second_quantity) === this.quantity_product_cart_first + 1) {
